...
| 画字字体 | 成品预览右上角切换:系统黑体 / 杨任东竹石体 / 沐瑶随心手写体;COS 主源 + 本地 `public/fonts/` 兜底,点选才按需加载,选择持久化,导出同款 |
...
├── vite.config.js
├── src/
│   ├── engine.js       # 渲染引擎:采样/掩码/边缘/平铺/绘制(可切换字体栈)
│   ├── fonts.js        # 画字字体:三档加载(COS 主源 + 本地兜底)/ 切换 / 持久化
│   ├── exporter.js     # 导出:PNG / jsPDF(A4)/ TXT 下载
│   ├── main.js         # 应用逻辑:上传/参数绑定/示例/预设/交互/字体切换
│   └── style.css       # 样式
└── public/
    ├── fonts/          # 子集化 woff2(思源宋体/霞鹜文楷)+ 手写 TTF 兜底(竹石体/沐瑶)
