...
| 功能 | 说明 |
| --- | --- |
| 图片输入 | 点击选择 / 拖拽 / `Ctrl+V` 粘贴截图 |
| 一键示例 | 球体、心形、卡通头像、Logo 四个内置样图 |
| 实时渲染 | 所有参数拖动即重绘(典型 100 列 < 100ms) |
| 渲染模式 | 单色(自选前景/背景色)/ 原色(按原图着色,自动保可读性) |
| 明暗控制 | 亮度阈值、对比度、亮度、反转、加粗边缘 |
...
│   └── style.css       # 样式
└── public/
    ├── samples/        # 一键示例图(与 backend demo 参数一致)
    ├── cases/          # 案例画廊图(词云等离线管线产物)
    └── favicon.svg
```
